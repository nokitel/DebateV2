...
3. Complete `Romarg_TODO.md` by replacing Romarg nameservers with those Cloudflare nameservers.
4. Run `make wait-dezbatere-dns`.
5. Run `make hosting-status`.
6. Run `cloudflared tunnel login` if `make interactive-manual-setup` did not already complete it.
7. Run `make resume-dezbatere-hosting`.
8. After manual HTTPS verification, run `INSTALL_SERVICE=1 STOP_QUICK_TUNNEL=1 make resume-dezbatere-hosting`.
